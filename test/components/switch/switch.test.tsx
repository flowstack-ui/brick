...
import { Switch, type SwitchSize } from "../../../src/switch.js";
...
describe("Switch", () => {
  it("renders adopted defaults, anatomy, slots, and refs", () => {
    ...
    render(<Switch.Root aria-label="Weekly reports" ref={rootRef}><Switch.Thumb ref={thumbRef} /></Switch.Root>);
  ...
  it("supports every closed size without leaking the visual prop", () => {
    ...
    const { rerender } = render(<Switch.Root aria-label="Reports"><Switch.Thumb /></Switch.Root>);
    for (const size of sizes) {
      rerender(<Switch.Root aria-label="Reports" size={size}><Switch.Thumb /></Switch.Root>);
  ...
  it("preserves controlled interaction and state on both parts", async () => {
    ...
    function Controlled() {
      ...
      return <Switch.Root aria-label="Reports" checked={checked} onCheckedChange={(next) => { changes(next); setChecked(next); }}><Switch.Thumb /></Switch.Root>;
    }
    render(<Controlled />);
    const root = screen.getByRole("switch");
    ...
    expect(root.querySelector("[data-slot='switch-thumb']")).toHaveAttribute("data-state", "checked");
  });

  it("keeps read-only focusable, disabled unavailable, and form state native", async () => {
    const user = userEvent.setup();
    const changes = vi.fn();
    render(<><Switch.Root aria-label="Read only" defaultChecked name="reports" onCheckedChange={changes} readOnly required value="enabled"><Switch.Thumb /></Switch.Root><Switch.Root aria-label="Disabled" disabled><Switch.Thumb /></Switch.Root></>);
  ...
  it("inherits Field relationships and preserves render/asChild", () => {
    render(<><Field.Root id="reports" invalid required><Field.Label>Weekly reports</Field.Label><Field.Description>Immediate delivery.</Field.Description><Switch.Root render={<button data-adapter="root" />}><Switch.Thumb render={<i data-adapter="thumb" />} /></Switch.Root><Field.Error>Required.</Field.Error></Field.Root><Switch.Root aria-label="Composed" asChild><button data-adapter="as-child"><Switch.Thumb /></button></Switch.Root></>);
    ...
    expect(fieldSwitch).toHaveAttribute("aria-describedby", "reports-description reports-error");
    expect(fieldSwitch.querySelector("[data-adapter='thumb']")?.tagName).toBe("I");
    expect(screen.getByRole("switch", { name: "Composed" })).toHaveAttribute("data-adapter", "as-child");
  });
});
