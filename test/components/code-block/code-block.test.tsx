...
describe("CodeBlock", () => {
  it("renders only authored anatomy and canonical pre/code structure", () => {
    ...
    render(
      <CodeBlock.Root ref={ref} value={source}>
        <CodeBlock.Content aria-label="Source" />
      </CodeBlock.Root>,
    );
    ...
    render(
      ...
      >
        <CodeBlock.Header>
          <CodeBlock.Title>Example</CodeBlock.Title>
          <CodeBlock.Language />
          <CodeBlock.Actions>Actions</CodeBlock.Actions>
        </CodeBlock.Header>
        <CodeBlock.Content aria-label="Highlighted source">
          <span className="token-keyword">const</span> answer = 42;
        </CodeBlock.Content>
      </CodeBlock.Root>,
    );
    expect(screen.getByText("TSX")).toHaveAttribute(
      "data-slot",
      "code-block-language",
    );
    const root = screen
  ...
  it("copies Root value rather than rendered highlighted text", async () => {
    ...
    render(
      <CodeBlock.Root value={source} writeValue={writeValue}>
        <CodeBlock.Content aria-label="Source">
          <span>different rendered tokens</span>
        </CodeBlock.Content>
        <CodeBlock.CopyTrigger>Copy</CodeBlock.CopyTrigger>
        <CodeBlock.CopyStatus>
  ...
  it("reports copy rejection truthfully", async () => {
    render(
      <CompleteBlock
        writeValue={vi.fn().mockRejectedValue(new Error("denied"))}
      />,
    );
    fireEvent.click(screen.getByRole("button", { name: "Copy" }));
  ...
  it("forwards part refs, native props, classes, slots, and wrap", () => {
    ...
    render(
      <CodeBlock.Root className="root-custom" data-owner="docs" value={source}>
        <CodeBlock.Header className="header-custom">
          <CodeBlock.Title data-slot="title-custom">Title</CodeBlock.Title>
        </CodeBlock.Header>
        <CodeBlock.Content
          className="content-custom"
          data-owner="source"
          data-slot="content-custom"
          focusable={false}
          ref={contentRef}
  ...
  it("renders trusted adapter output and independent line metadata", () => {
    const adapter = vi.fn(({ value }: { value: string }) =>
      value.split("\n").map((line, index) => (
        <CodeBlock.Line
    ...
    const lines = screen
      .getByRole("region", { name: "Adapted source" })
      .querySelectorAll("[data-slot='code-block-line']");
    ...
    expect(
      lines[1].querySelector("[data-slot='code-block-line-content']"),
    ).toHaveTextContent("const second = 2;");
  });
  ...
  it("rejects injected HTML on Root, Content, and Line for untyped consumers", () => {
    ...
    expect(
      screen.getByRole("region", { name: "Safe source" }),
    ).toHaveTextContent("safe line");
  });
  ...
  it("lets explicit Content children override the optional adapter", () => {
    ...
    render(
      <CodeBlock.Root adapter={adapter} value={source}>
        <CodeBlock.Content aria-label="Explicit source">
          explicit output
        </CodeBlock.Content>
      </CodeBlock.Root>,
    );
    expect(
      screen.getByRole("region", { name: "Explicit source" }),
    ).toHaveTextContent("explicit output");
  ...
  it("bounds positive whole line counts and composes Collapsible disclosure state", () => {
      ...
      <CodeBlock.Collapse>
        <CodeBlock.Root adapter={adapter} value={source}>
          <CodeBlock.CollapsePreview>
            <CodeBlock.Content
              aria-label="Bounded source preview"
              maxLines={4}
            />
          </CodeBlock.CollapsePreview>
    ...
    expect(trigger).toHaveAccessibleName("Hide full source");
    expect(trigger).toHaveAttribute("aria-expanded", "true");
    expect(
      document.getElementById(trigger.getAttribute("aria-controls")!),
    ).toHaveAttribute("data-slot", "code-block-collapse-content");
    expect(
      screen.getByRole("region", { name: "Full bounded source" }),
    ).toBeVisible();
