...
describe("OTP Field",()=>{
 it("uses the shared default and responsive size metadata",()=>{render(<OTPField.Root length={4} size={{initial:"sm",lg:"xl"}}><OTPField.Group>{cells()}</OTPField.Group></OTPField.Root>);const root=document.querySelector(".brick-otp-field");expect(root).toHaveAttribute("data-size","sm");expect(root).toHaveAttribute("data-size-lg","xl");});
