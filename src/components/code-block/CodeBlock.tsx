...
import { useCollapsibleContext as useAtomCollapsibleContext } from "@flowstack-ui/atom/collapsible";
import {
  forwardRef,
  useMemo,
  type CSSProperties,
...
export interface CodeBlockRootProps extends Omit<
  ...
  "dangerouslySetInnerHTML" | "defaultValue" | "value"
> {
  value: string;
  language?: string;
...
export interface CodeBlockLanguageProps extends SlottedProps<
  HTMLAttributes<HTMLSpanElement>
> {
  children?: ReactNode;
}
export type CodeBlockActionsProps = SlottedProps<
  HTMLAttributes<HTMLDivElement>
>;
export interface CodeBlockContentProps extends Omit<
  SlottedProps<HTMLAttributes<HTMLDivElement>>,
...
export interface CodeBlockLineProps extends Omit<
  SlottedProps<HTMLAttributes<HTMLSpanElement>>,
  "dangerouslySetInnerHTML"
> {
  lineNumber?: number;
  highlighted?: boolean;
...
export type CodeBlockCollapsePreviewProps = SlottedProps<
  HTMLAttributes<HTMLDivElement>
>;
...
function withoutInjectedHtml<T extends object>(
  props: T,
): Omit<T, "dangerouslySetInnerHTML"> {
  const { dangerouslySetInnerHTML: _blockedHtml, ...safeProps } = props as T & {
...
export const CodeBlockHeader = forwardRef<HTMLDivElement, CodeBlockHeaderProps>(
  function CodeBlockHeader(
    ...
    return (
      <div
        {...props}
        className={classes("brick-code-block-header", className)}
        data-slot={slot(dataSlot, "code-block-header")}
        ref={ref}
      />
    );
  },
...
export const CodeBlockTitle = forwardRef<HTMLSpanElement, CodeBlockTitleProps>(
  function CodeBlockTitle({ className, "data-slot": dataSlot, ...props }, ref) {
    return (
      <span
        {...props}
        className={classes("brick-code-block-title", className)}
        data-slot={slot(dataSlot, "code-block-title")}
        ref={ref}
      />
    );
  },
...
export const CodeBlockLanguage = forwardRef<
  HTMLSpanElement,
...
>(function CodeBlockLanguage(
  ...
  return (
    <span
      {...props}
      className={classes("brick-code-block-language", className)}
      data-slot={slot(dataSlot, "code-block-language")}
      ref={ref}
    >
      {children ?? language}
    </span>
...
export const CodeBlockActions = forwardRef<
  HTMLDivElement,
...
>(function CodeBlockActions(
  ...
  return (
    <div
      {...props}
      className={classes("brick-code-block-actions", className)}
      data-slot={slot(dataSlot, "code-block-actions")}
      ref={ref}
    />
  );
});
...
>(function CodeBlockContent(
  ...
  const boundedLines =
    Number.isInteger(maxLines) && Number(maxLines) > 0
      ? Number(maxLines)
      : undefined;
  ...
  const contentStyle = boundedLines
    ? ({
        ...style,
        "--brick-code-block-max-block-size": `${boundedLines}lh`,
      } as CSSProperties)
    : style;
  return (
    <ScrollArea.Root
      className="brick-code-block-scroll-area"
...
export const CodeBlockLine = forwardRef<HTMLSpanElement, CodeBlockLineProps>(
  function CodeBlockLine(
    ...
    const visibleLineNumber =
      Number.isInteger(lineNumber) && Number(lineNumber) > 0
        ? Number(lineNumber)
        : undefined;
    return (
      <span
      ...
      >
        <span
          className="brick-code-block-line-content"
...
export const CodeBlockCollapse = forwardRef<
  HTMLDivElement,
  CodeBlockCollapseProps
>(function CodeBlockCollapse(
  { className, "data-slot": dataSlot, ...props },
  ref,
) {
  return (
...
export const CodeBlockCollapsePreview = forwardRef<
  HTMLDivElement,
  CodeBlockCollapsePreviewProps
>(function CodeBlockCollapsePreview(
  { className, "data-slot": dataSlot, ...props },
  ref,
) {
  return (
...
export const CodeBlockCollapseContent = forwardRef<
  HTMLDivElement,
  CodeBlockCollapseContentProps
>(function CodeBlockCollapseContent(
...
>(function CodeBlockCollapseTrigger(
  ...
  const hasStateLabels = closedLabel !== undefined || openLabel !== undefined;
  const { isOpen } = useAtomCollapsibleContext();
  return (
    <Collapsible.Trigger
      {...props}
    ...
    >
      {hasStateLabels
        ? isOpen
          ? (openLabel ?? children)
          : (closedLabel ?? children)
        : children}
    </Collapsible.Trigger>
  );
});

export const CodeBlockCopyTrigger = forwardRef<
  HTMLElement,
...
export const CodeBlockCopyIndicator = forwardRef<
  HTMLSpanElement,
  CodeBlockCopyIndicatorProps
>(function CodeBlockCopyIndicator(
  { className, "data-slot": dataSlot, ...props },
  ref,
) {
  return (
    <AtomClipboard.Indicator
      {...props}
      className={classes("brick-code-block-copy-indicator", className)}
      data-slot={slot(dataSlot, "code-block-copy-indicator")}
      ref={ref}
    />
  );
});

export const CodeBlockCopyStatus = forwardRef<
  HTMLSpanElement,
  CodeBlockCopyStatusProps
>(function CodeBlockCopyStatus(
  { className, "data-slot": dataSlot, ...props },
  ref,
) {
  return (
    <AtomClipboard.Status
      {...props}
      className={classes("brick-code-block-copy-status", className)}
      data-slot={slot(dataSlot, "code-block-copy-status")}
      ref={ref}
    />
  );
});
