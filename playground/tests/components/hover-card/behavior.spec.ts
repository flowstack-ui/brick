...
test("HoverCard removes authored motion when reduced motion is requested", async ({
  page,
}) => {
...
test("HoverCard remains open across the pointer bridge and contains no interactive descendants", async ({
  page,
  isMobile,
}) => {
  test.skip(
  ...
  const content = page
    .locator("[data-slot='hover-card']")
    .filter({ hasText: "compiler pioneer" });
  await expect(content).toBeVisible();
  await content.hover();
  await page.waitForTimeout(150);
  await expect(content).toBeVisible();
  await expect(
    content.locator("a,button,input,select,textarea,[tabindex]"),
  ).toHaveCount(0);
});

test("HoverCard does not open from touch and preserves native link navigation", async ({
  page,
}) => {
  ...
  await expect(page.getByTestId("hover-card-content-ada-lovelace")).toHaveCount(
    0,
  );
  await trigger.click();
...
test("HoverCard exposes the three bounded sizes, optional shared Arrow, and disabled state", async ({
  page,
  isMobile,
}) => {
  await page.goto("/hover-card");
  ...
  await expect(noArrow.locator("[data-slot='hover-card-arrow']")).toHaveCount(
    0,
  );
  await page.keyboard.press("Escape");
...
test("HoverCard keeps its genuine link contract and passes the focused accessibility audit", async ({
  ...
  await expect(link).toHaveAttribute(
    "href",
    "/hover-card/destination?resource=compiler-project-notes",
  );
  const output = page
...
test("HoverCard remains contained at 256 px and supports RTL logical layout", async ({
  page,
}) => {
...
test("HoverCard can fall back to a perpendicular axis when neither horizontal side fits", async ({
  page,
}) => {
  await page.setViewportSize({ width: 256, height: 640 });
  await page.goto("/hover-card");
...
test("HoverCard preserves the requested side when another alignment fits", async ({
  page,
}) => {
