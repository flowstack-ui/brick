...
test("the four qualified constraint families resolve without prop leakage", async ({ page }) => {
...
test("responsive constraints carry forward through standard breakpoints", async ({ page }) => {
  const frame = page.getByTestId("frame-responsive");
  await page.setViewportSize({ width: 390, height: 844 });
  await expect(frame).toHaveCSS("width", "358px");
  await expect(frame).toHaveCSS("min-height", "128px");
  await page.setViewportSize({ width: 800, height: 900 });
  expect(await frame.evaluate((element) => getComputedStyle(element).width)).toMatch(/px$/);
  await expect(frame).toHaveCSS("min-height", "128px");
  await page.setViewportSize({ width: 1280, height: 900 });
...
test("nested Frames do not inherit any parent constraints", async ({ page }) => {
...
test("logical stress and axe remain valid", async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await expect(page.locator('.brick-frame[dir="rtl"]')).toHaveCSS("direction", "rtl");
  await expect(page.locator(".frame-vertical")).toHaveCSS("writing-mode", "vertical-rl");
