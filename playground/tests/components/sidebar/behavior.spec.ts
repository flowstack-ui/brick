...
test("panel surface selection remains independent from docked or floating geometry", async ({
  page,
}) => {
  const scenario = page.locator('[data-scenario="sidebar.appearance"]');
  const transparent = scenario.locator(
    '.brick-sidebar[data-surface="transparent"]',
  );
  await expect(transparent).toHaveAttribute("data-variant", "docked");
  await expect(transparent.locator(".brick-sidebar__panel")).toHaveCSS(
    "background-color",
    "rgba(0, 0, 0, 0)",
  );
});

test("controlled Trigger changes state and offcanvas output remains inert", async ({
  page,
}) => {
  const scenario = page.locator('[data-scenario="sidebar.behavior"]');
  const trigger = scenario.getByRole("button", {
    name: "Toggle controlled sidebar",
  });
  ...
  await expect(output.locator(".brick-sidebar__panel")).toHaveAttribute(
    "inert",
    "",
  );
  await expect(output.locator("[data-rendered-output]")).toContainText(
    "aria-hidden",
  );
});

test("right side, sticky containment, composition, mobile reflow, and accessibility pass", async ({
  page,
}) => {
  const right = page.locator(
    '[data-scenario="sidebar.placement"] .brick-sidebar[data-side="right"]',
  );
  ...
  await expect(
    page.locator('[data-scenario="sidebar.regions"] [data-rendered-output]'),
  ).not.toHaveText("");
  await page.setViewportSize({ width: 390, height: 844 });
  const stress = page.locator('[data-scenario="sidebar.stress"]');
  const box = await stress.boundingBox();
  expect(box!.x + box!.width).toBeLessThanOrEqual(390);
  const results = await new AxeBuilder({ page })
    .include('[data-testid="sidebar-overview"]')
    .analyze();
  expect(results.violations).toEqual([]);
});

test("desktop playground shell consumes Sidebar while mobile policy remains Drawer", async ({
  page,
}) => {
...
test("Sidebar preserves logical navigation and coordinated geometry in RTL", async ({
  page,
}) => {
  const scenario = page.locator('[data-scenario="sidebar.stress"] [dir="rtl"]');
  const root = scenario.locator(".brick-sidebar");
  const panel = root.locator(".brick-sidebar__panel");
  const main = root.locator(".brick-sidebar__main");
  ...
  const mainBox = await main.boundingBox();
  expect(panelBox!.x).toBeGreaterThanOrEqual(mainBox!.x + mainBox!.width - 1);
  await root.getByRole("link", { name: "Overview" }).focus();
  await expect(root.getByRole("link", { name: "Overview" })).toBeFocused();
});
