...
test("Button exposes the approved recipes and native semantics", async ({ page }) => {
  ...
  const output = page.getByTestId("button-composition").locator("[data-rendered-output]");
  ...
  await expect(
    page.locator("#scenario-button-states"),
  ).toBeInViewport();
});
...
test("Button native actions compose with Brick Form and Field", async ({
  ...
  await expect(
    form.getByRole("button", { name: "Reset" }),
  ...
  await expect(form.getByText('Submitted "Documentation refresh".')).toBeVisible();
...
test("Button keeps loading centered and directional icons semantic in RTL", async ({
  ...
  const readSpinnerCenter = () => loading.evaluate((element) => {
  ...
  for (const { position, testId } of [
    ...
    const [iconBox, contentBox, transform] = await Promise.all([
      icon.boundingBox(),
      content.boundingBox(),
      icon.locator("svg").evaluate((element) => getComputedStyle(element).transform),
    ]);
...
test("Button evidence uses complete tone groups and balanced variant rows", async ({
  ...
  const [toneScenarioGap, toneStackGap] = await Promise.all([
    page.locator("[data-scenario='button.tones']").evaluate(
      (element) => getComputedStyle(element).rowGap,
    ),
    page.getByTestId("button-tones").evaluate(
      (element) => getComputedStyle(element).rowGap,
    ),
  ]);
