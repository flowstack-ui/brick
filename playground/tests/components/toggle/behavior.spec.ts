...
test("Toggle overview preserves the default recipe and native pressed state", async ({ page }) => {
...
test("Toggle comparison scenarios change only their named dimensions", async ({ page }) => {
...
test("Toggle composition, disabled state, customization, and RTL remain observable", async ({ page }) => {
  await expect(page.getByTestId("toggle-render")).toHaveAttribute("aria-pressed", "false");
  await expect(page.getByTestId("toggle-as-child")).toHaveAttribute("aria-pressed", "false");
  const output = page
