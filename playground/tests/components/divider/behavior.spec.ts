...
test("default and semantics preserve the adopted Atom contract", async ({
  ...
  const defaultPaint = await divider.evaluate((element) => {
    const root = getComputedStyle(document.documentElement);
    return {
      expected: root.getPropertyValue("--brick-color-border-default").trim(),
      actual: getComputedStyle(element)
        .getPropertyValue("--brick-divider-color")
        .trim(),
    };
  });
  expect(defaultPaint.actual).toBe(defaultPaint.expected);
  const semantic = page
    .getByTestId("divider-semantics")
    .locator('[role="separator"]');
  await expect(semantic).toHaveAttribute("aria-label", "Archived workspace");
});

test("orientation, variants, thickness, inset, and labels are controlled", async ({
  page,
}) => {
  const vertical = page
    .getByTestId("divider-orientations")
  ...
  const styles = await page
    .getByTestId("divider-variants")
    .locator(".brick-divider")
    .evaluateAll((items) =>
      items.map((item) => getComputedStyle(item).borderTopStyle),
    );
  expect(styles).toEqual(["solid", "dashed", "dotted"]);
  const widths = await page
    .getByTestId("divider-thicknesses")
    .locator(".brick-divider")
    .evaluateAll((items) =>
...
test("composition, customization, RTL, reflow, and accessibility remain sound", async ({
  page,
}) => {
  await expect(
    page.getByTestId("divider-semantics").locator("[data-rendered-output]"),
  ).toContainText("Composed section boundary");
  await page.getByRole("button", { name: "Inspect ref" }).click();
  await expect(page.getByText("Ref host: HR")).toBeVisible();
  const custom = page
    .getByTestId("divider-appearance")
    .locator(".playground-customization-preview .brick-divider");
  expect(
  ...
  const results = await new AxeBuilder({ page })
    .include('[data-component-page="divider"]')
    .analyze();
