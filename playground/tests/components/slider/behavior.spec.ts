...
test("defaults, recipes, ranges, Field states, and form output are complete",async({page})=>{const thumb=page.getByTestId("slider-overview").getByRole("slider",{name:"Volume"});await expect(thumb).toHaveAttribute("aria-valuenow","40");await expect(thumb.locator(".brick-slider__value-label")).toHaveCount(0);const expectedGeometry={sm:{thumb:16,track:6},md:{thumb:20,track:8},lg:{thumb:24,track:10}} as const;for(const size of ["sm","md","lg"] as const){const root=page.getByTestId("slider-recipes").locator(`.brick-slider[data-size='${size}']`).first();await expect(root).toHaveCount(1);const geometry=await root.evaluate((node)=>({thumb:Number.parseFloat(getComputedStyle(node.querySelector(".brick-slider__thumb")!,"::before").width),track:Number.parseFloat(getComputedStyle(node.querySelector(".brick-slider__track")!,"::before").height)}));expect(geometry).toEqual(expectedGeometry[size])}await expect(page.getByTestId("slider-values").getByRole("slider")).toHaveCount(3);await expect(page.getByTestId("slider-states").locator(".brick-slider[data-invalid]")).toHaveCount(1)});
test("large thumb artwork inherits the polished Slider content scale",async({page})=>{const thumb=page.getByTestId("slider-content").getByRole("slider",{name:"Seats"});await expect(thumb.locator(".brick-icon")).toHaveAttribute("data-size","inherit");const geometry=await thumb.evaluate((node)=>{const icon=node.querySelector(".brick-icon")!;return{thumb:Number.parseFloat(getComputedStyle(node,"::before").width),icon:Number.parseFloat(getComputedStyle(icon).width),shadow:getComputedStyle(node,"::before").boxShadow}});expect(geometry).toEqual({thumb:24,icon:14,shadow:"none"})});
