...
function DestinationIcon({ children }: { children: ReactNode }) {
  return <BottomNavigation.Icon><Icon size="md">{children as ReactElement}</Icon></BottomNavigation.Icon>;
}
  ...
  return createElement(
    ...
    destinations.slice(0, count).map((destination) => (
        <BottomNavigation.Item href={`#bottom-navigation-${destination.value}`} key={destination.value} value={destination.value}>
          {notification && destination.value === "inbox" ? <NotificationBadge count={4}><DestinationIcon>{destination.icon}</DestinationIcon></NotificationBadge> : <DestinationIcon>{destination.icon}</DestinationIcon>}
