...
function StructuredList({ dir }: { dir?: "ltr" | "rtl" }) {
  return <List.Root dir={dir} marker="none" variant="divided">
    <List.Item>
...
export const listScenarios = [
  ...
  { id: "list.sizing", number: 4, title: "Sizes and density", navigationTitle: "Sizing", description: "Size changes typography and metrics, while density independently changes only vertical row spacing." },
...
export function ListPage() {
  return <VStack className="list-page" data-component-page="list" gap="6">
    ...
    <Scenario {...listScenarios[1]}><VStack gap="4"><Grid.Root className="list-grid list-grid--two" columns={2} gap="4"><Cell label="unordered"><BasicList /></Cell><Cell label="ordered"><BasicList ordered /></Cell></Grid.Root><Grid.Root className="list-output-grid" columns={2} gap="4"><RenderedOutput label="Unordered List HTML"><BasicList /></RenderedOutput><RenderedOutput label="Ordered List HTML"><BasicList ordered /></RenderedOutput></Grid.Root></VStack></Scenario>
    ...
    <Scenario {...listScenarios[7]}><VStack gap="5"><Grid.Root className="list-grid list-grid--two" columns={2} gap="4"><EvidenceSurface data-brick-appearance="light"><SpecimenLabel>light</SpecimenLabel><BasicList variant="bordered" /></EvidenceSurface><EvidenceSurface data-brick-appearance="dark"><SpecimenLabel>dark</SpecimenLabel><BasicList variant="bordered" /></EvidenceSurface></Grid.Root><EvidenceSurface><Grid.Root className="list-customization" columns={2} gap="5"><VStack gap="2"><Text as="h3" variant="title-sm">List CSS properties</Text><Text tone="secondary" variant="body-sm">The preview uses the exact accent border, marker, radius, and row padding shown in code.</Text><PlaygroundCodeBlock tabIndex={0}>{`--brick-list-border-color: var(--brick-color-accent-border);\n--brick-list-marker-color: var(--brick-color-accent-solid);\n--brick-list-radius: 1.25rem;\n--brick-list-row-padding-block: var(--brick-space-4);`}</PlaygroundCodeBlock></VStack><BasicList style={customStyle} variant="bordered" /></Grid.Root></EvidenceSurface></VStack></Scenario>
    <Scenario {...listScenarios[8]}><Grid.Root className="list-grid list-grid--stress" columns={2} gap="4"><Cell label="narrow and long"><div className="list-narrow"><List.Root><List.Item>A carefully localized requirement wraps without escaping its constrained list container.</List.Item><List.Item>Package verification remains readable at narrow widths.</List.Item></List.Root></div></Cell><Cell label="RTL structured"><StructuredList dir="rtl" /></Cell><Cell label="marker-free"><BasicList marker="none" /></Cell><Cell label="long trailing row"><List.Root marker="none" variant="bordered"><List.Item><List.Content><List.Title>International workspace publishing review</List.Title><List.Description>Supporting copy wraps while the independently named action stays contained.</List.Description></List.Content><List.Trailing><Button size="sm">Review</Button></List.Trailing></List.Item></List.Root></Cell></Grid.Root></Scenario>
  </VStack>;
}
