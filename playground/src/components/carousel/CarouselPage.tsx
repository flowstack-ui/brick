...
function Picker() {
  return <Carousel.Picker>{slides.map(slide => <Carousel.PickerItem key={slide.value} value={slide.value} />)}</Carousel.Picker>;
}

function FullCarousel(props: Omit<React.ComponentProps<typeof Carousel.Root>, "children"> = {}) {
  return <Carousel.Root aria-label="Flowstack story" defaultValue="build" {...props}><Slides /><Carousel.Navigation><Carousel.Previous /><Carousel.Next /></Carousel.Navigation><Carousel.Controls>{props.defaultAutoPlay ? <Carousel.RotationControl /> : null}<Picker /></Carousel.Controls></Carousel.Root>;
}
...
export function CarouselPage() {
  return <VStack data-component-page="carousel" gap="6">
    ...
    <Scenario {...carouselScenarios[3]}><Grid.Root className="carousel-demo-grid" columns={2} gap="4"><FullCarousel size="sm" /><Surface className="carousel-demo-dark" data-brick-appearance="dark" inset="md" level="canvas"><FullCarousel controlPlacement="outside" size="lg" /></Surface></Grid.Root></Scenario>
  </VStack>;
}
