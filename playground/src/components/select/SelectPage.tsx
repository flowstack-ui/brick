  ...
  return (
    <Field.Root id={id}>
      ...
      <Select.Root {...selectProps}>
        ...
        <Select.Content disablePortal={id === "select-long"} style={contentStyle}>
...
function Tile({ label, children }: { label: string; children: ReactNode }) {
  return <EvidenceSurface className="select-tile" inset="md"><SpecimenLabel>{label}</SpecimenLabel>{children}</EvidenceSurface>;
}
...
export function SelectPage() {
  ...
  return (
    <VStack className="forms-page select-page" data-component-page="select" data-testid="select-workbench">
      ...
      <Scenario {...selectScenarios[1]}><Grid.Root columns={3} className="forms-grid forms-grid--three" data-testid="select-variants">{variants.map((variant) => <Tile key={variant} label={variant}><PlanSelect id={`select-variant-${variant}`} rootProps={{ variant }} /></Tile>)}</Grid.Root></Scenario>
      <Scenario {...selectScenarios[2]}><Grid.Root columns={3} className="forms-grid forms-grid--three" data-testid="select-sizes">{sizes.map((size) => <Tile key={size} label={size}><PlanSelect id={`select-size-${size}`} rootProps={{ size }} /></Tile>)}</Grid.Root></Scenario>
      <Scenario {...selectScenarios[3]}><VStack gap="4" data-testid="select-shapes"> <Grid.Root columns={3} className="forms-grid forms-grid--three">{shapes.map((shape) => <Tile key={shape} label={shape}><PlanSelect id={`select-shape-${shape}`} rootProps={{ shape }} /></Tile>)}</Grid.Root><Grid.Root columns={2} className="forms-grid"><Tile label="full width"><PlanSelect id="select-full-width" /></Tile><Tile label="intrinsic"><PlanSelect id="select-intrinsic" rootProps={{ fullWidth: false }} /></Tile></Grid.Root></VStack></Scenario>
      <Scenario {...selectScenarios[4]}><EvidenceSurface data-testid="select-options" inset="lg"><PlanSelect id="select-long" rootProps={{ defaultOpen: true }}><Select.Group><Select.Label>Recommended</Select.Label>{Options()}</Select.Group><Select.Separator /><Select.Group><Select.Label>Regions</Select.Label>{Array.from({ length: 14 }, (_, index) => <Select.Item key={index} value={`region-${index}`}><Select.ItemText>Region {index + 1}</Select.ItemText><Select.ItemIndicator /></Select.Item>)}</Select.Group></PlanSelect></EvidenceSurface></Scenario>
      <Scenario {...selectScenarios[5]}><Grid.Root columns={3} className="forms-grid forms-grid--three" data-testid="select-states"><Tile label="placeholder"><Field.Root id="select-placeholder"><Field.Label>Plan</Field.Label><Select.Root><Select.Trigger><Select.Value placeholder="Choose a plan" /><Select.Icon /></Select.Trigger><Select.Content><Select.Viewport><Options /></Select.Viewport><Select.Arrow /></Select.Content></Select.Root></Field.Root></Tile><Tile label="controlled"><Field.Root id="select-controlled"><Field.Label>Plan</Field.Label><Select.Root value={value} onValueChange={setValue} open={open} onOpenChange={setOpen}><Select.Trigger><Select.Value /><Select.Icon /></Select.Trigger><Select.Content><Select.Viewport><Options /></Select.Viewport><Select.Arrow /></Select.Content></Select.Root><Text variant="caption">Value: {value}; open: {String(open)}</Text></Field.Root></Tile><Tile label="disabled"><PlanSelect id="select-disabled" rootProps={{ disabled: true }} /></Tile><Tile label="read only"><PlanSelect id="select-readonly" rootProps={{ readOnly: true }} /></Tile><Tile label="invalid"><Field.Root id="select-invalid" invalid><Field.Label>Plan</Field.Label><Select.Root invalid><Select.Trigger><Select.Value placeholder="Choose a plan" /><Select.Icon /></Select.Trigger><Select.Content><Select.Viewport><Options /></Select.Viewport></Select.Content></Select.Root><Field.Error>Choose an available plan.</Field.Error></Field.Root></Tile></Grid.Root></Scenario>
      <Scenario {...selectScenarios[6]}><Grid.Root columns={2} className="forms-grid" data-testid="select-forms"><Tile label="native form"><Form onReset={() => setStatus("Form reset")} onSubmit={(event) => { event.preventDefault(); setStatus(String(new FormData(event.currentTarget).get("plan"))); }}><Field.Root id="select-form-plan" required><Field.Label>Billing plan</Field.Label><Select.Root defaultValue="team" name="plan" required><Select.Trigger><Select.Value placeholder="Choose a plan" /><Select.Icon /></Select.Trigger><Select.Content><Select.Viewport>{Options()}</Select.Viewport><Select.Arrow /></Select.Content></Select.Root><Field.Error>Choose a plan.</Field.Error></Field.Root><HStack><Button type="submit">Submit</Button><Button tone="neutral" type="reset">Reset</Button></HStack><output data-testid="select-form-status"><Text as="span" variant="caption">{status}</Text></output></Form></Tile><Tile label="asChild trigger"><Field.Root id="select-as-child"><Field.Label>Plan</Field.Label><Select.Root defaultValue="team"><Select.Trigger asChild><button type="button"><Select.Value /><Select.Icon /></button></Select.Trigger><Select.Content><Select.Viewport>{Options()}</Select.Viewport><Select.Arrow /></Select.Content></Select.Root></Field.Root></Tile></Grid.Root></Scenario>
      <Scenario {...selectScenarios[7]}><Grid.Root columns={3} className="forms-grid forms-grid--three" data-testid="select-appearance"><Tile label="light"><div data-appearance="light"><PlanSelect id="select-light" /></div></Tile><Tile label="dark"><div data-appearance="dark"><PlanSelect id="select-dark" /></div></Tile><Tile label="customized"><PlanSelect id="select-custom" contentStyle={customTokens} /><Text variant="caption">Trigger, popup, and highlight use documented Select variables.</Text></Tile></Grid.Root></Scenario>
      <Scenario {...selectScenarios[8]}><Grid.Root columns={2} className="forms-grid" data-testid="select-stress"><Tile label="narrow and long"><div className="select-narrow"><PlanSelect id="select-long-text" label="Localized subscription plan with extended wording"><Select.Item value="team"><Select.ItemText>Collaboration workspace with an intentionally long localized option</Select.ItemText><Select.ItemIndicator /></Select.Item></PlanSelect></div></Tile><Tile label="RTL"><div dir="rtl"><PlanSelect id="select-rtl" label="الخطة" /></div></Tile></Grid.Root></Scenario>
    </VStack>
  );
}
