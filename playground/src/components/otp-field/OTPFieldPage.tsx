...
import { Badge, Button, Field, Fieldset, Form, Grid, OTPField, Text, VStack, type OTPFieldLayout, type OTPFieldShape, type OTPFieldSize, type OTPFieldVariant } from "@flowstack-ui/brick";
...
const definitions = [
  ...
  [7, "Form, Fieldset, and Field", "Native submission, reset, validation, and inherited group state."],
...
export function OTPFieldPage() {
  ...
  return <VStack className="forms-page specialized-field-page otp-field-page" data-component-page="otp-field">
    ...
    <Scenario {...otpFieldScenarios[6]}><VStack className="forms-evidence-stack"><EvidenceGroup title="Native form and inherited Fieldset state" description="The required Fieldset passes validation context while submit and reset preserve native form behavior."><EvidenceSurface className="forms-overview" data-testid="otp-field-form"><Form aria-label="Verification form" preventDefaultOnSubmit validationBehavior="inline" onReset={() => setStatus("Form reset")} onSubmit={event => setStatus(`Submitted: ${new FormData(event.currentTarget).get("verification") ?? "none"}`)}><Fieldset.Root required><Fieldset.Legend>Identity verification</Fieldset.Legend><Field.Root id="otp-form-code"><Field.Label>Security code</Field.Label><Control length={4} name="verification" /><Field.Error>Enter the security code.</Field.Error></Field.Root></Fieldset.Root><Button type="submit">Verify</Button><Button type="reset" variant="outline">Reset</Button><output className="forms-status"><Text as="span">{status}</Text></output></Form></EvidenceSurface></EvidenceGroup></VStack></Scenario>
    <Scenario {...otpFieldScenarios[7]}><VStack className="forms-evidence-stack" data-testid="otp-field-appearance"><EvidenceGroup title="Scoped appearances" description="Compact badges identify identical OTP Field defaults inside light and dark scopes."><Grid.Root columns={2} className="forms-scoped-grid"><EvidenceSurface data-brick-appearance="light"><Badge size="sm">Light</Badge><Labeled id="otp-light"><Control length={4} /></Labeled></EvidenceSurface><EvidenceSurface data-brick-appearance="dark"><Badge size="sm">Dark</Badge><Labeled id="otp-dark"><Control length={4} /></Labeled></EvidenceSurface></Grid.Root></EvidenceGroup><EvidenceGroup title="Consumer customization" description="The titled preview uses only the documented public variables shown beside it."><EvidenceSurface as="article" className="forms-customization" inset="lg"><div><Badge size="sm" tone="accent">Customized</Badge><Text as="h4" variant="title-sm">Verification accent</Text><Text as="p" tone="secondary" variant="body-sm">Larger cells and an accent radius are scoped to this one verification field.</Text><PlaygroundCodeBlock tabIndex={0}>{`--brick-otp-size: 3.25rem;\n--brick-otp-radius: 1rem;`}</PlaygroundCodeBlock></div><EvidenceSurface className="forms-customization__preview"><Labeled id="otp-custom"><Control length={4} style={customTokens} /></Labeled></EvidenceSurface></EvidenceSurface></EvidenceGroup></VStack></Scenario>
