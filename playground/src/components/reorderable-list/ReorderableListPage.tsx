...
function ArrowGraphic({ direction }: { direction: "before" | "after" }) {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24">
      <path
        d={direction === "before" ? "m18 15-6-6-6 6" : "m6 9 6 6 6-6"}
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
...
function Cell({ children, label }: { children: ReactNode; label: string }) {
  return (
    <Specimen className="reorderable-list-cell" label={label}>
      {children}
    </Specimen>
  );
}
...
export const reorderableListScenarios = [
  ...
  },
  {
    ...
    title: "Recipes",
    description:
      "Outline and soft change paint, while sm, md, and lg change spacing without shrinking any movement target below 44px.",
  },
  {
    ...
    >
      ...
      <Scenario {...reorderableListScenarios[1]}>
        <VStack gap="5">
          ...
          <Grid.Root
            className="reorderable-list-grid reorderable-list-grid--sizes"
            columns={3}
            gap="4"
          >
            {sizes.map((size) => (
      ...
      <Scenario {...reorderableListScenarios[6]}>
        <VStack gap="5">
          ...
          <Cell label="320px long content">
            <div className="reorderable-list-narrow">
              <ReorderExample />
            </div>
          </Cell>
        </VStack>
