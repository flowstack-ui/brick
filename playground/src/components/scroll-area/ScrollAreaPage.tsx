...
function EvidenceGroup({ children, description, title }: { children: ReactNode; description: string; title: string }) {
  return <VStack as="section" gap="3"><VStack gap="1"><Text as="h3" variant="title-sm">{title}</Text><Text as="p" tone="secondary" variant="body-sm">{description}</Text></VStack>{children}</VStack>;
}
...
export function ScrollAreaPage() {
  ...
  return <VStack className="scroll-area-page" data-component-page="scroll-area" gap="6">
    ...
    <Scenario {...scrollAreaScenarios[7]}><VStack gap="4"><Grid.Root className="scroll-area-grid" columns={2}><EvidenceSurface className="scroll-area-cell" data-brick-appearance="light"><SpecimenLabel>Light</SpecimenLabel><Demo label="Light activity" /></EvidenceSurface><EvidenceSurface className="scroll-area-cell" data-brick-appearance="dark"><SpecimenLabel>Dark</SpecimenLabel><Demo label="Dark activity" /></EvidenceSurface></Grid.Root><EvidenceSurface className="playground-customization-evidence scroll-area-customization" inset="none"><Grid.Root className="playground-customization-layout" columns={2} gap="0"><VStack gap="2"><SpecimenLabel>Customized</SpecimenLabel><Text as="h3" variant="title-sm">Scrollbar CSS properties</Text><Text tone="secondary" variant="body-sm">Accent thumb and soft track change only the documented native scrollbar colors.</Text><PlaygroundCodeBlock tabIndex={0}>{`.custom-scroll-area {
  --brick-scroll-area-scrollbar-thumb: var(--brick-color-accent-solid);
  --brick-scroll-area-scrollbar-track: var(--brick-color-accent-soft);
}`}</PlaygroundCodeBlock></VStack><div className="playground-customization-preview"><Demo label="Customized activity" scrollbarVisibility="always" testId="scroll-area-custom" /></div></Grid.Root></EvidenceSurface></VStack></Scenario>
    <Scenario {...scrollAreaScenarios[8]}><VStack gap="6"><EvidenceGroup title="Responsive boundaries" description="The constrained horizontal viewport remains touch-scrollable without widening the page."><EvidenceSurface><div className="scroll-area-phone"><Demo label="Project timeline" orientation="horizontal"><HStack className="scroll-area-rtl-row" gap="3">{Array.from({ length: 8 }, (_, index) => <Badge key={index}>Project {index + 1}</Badge>)}</HStack></Demo></div></EvidenceSurface></EvidenceGroup><EvidenceGroup title="RTL inheritance" description="Horizontal scrolling and authored content follow genuine right-to-left direction without changing source order."><EvidenceSurface><div className="scroll-area-phone" dir="rtl"><Demo label="RTL project timeline" orientation="horizontal"><HStack className="scroll-area-rtl-row" gap="3">{Array.from({ length: 8 }, (_, index) => <Badge key={index}>المشروع {index + 1}</Badge>)}</HStack></Demo></div></EvidenceSurface></EvidenceGroup><EvidenceGroup title="Vertical writing" description="A vertical writing mode keeps the native horizontal viewport contained and readable."><EvidenceSurface><div className="scroll-area-vertical-writing"><Demo label="Vertical project notes" orientation="horizontal"><Text>縦書きのプロジェクト履歴を横方向に読み進めるための領域です。</Text></Demo></div></EvidenceSurface></EvidenceGroup></VStack></Scenario>
  </VStack>;
}
