...
export function FeedPage() {
  ...
  return (
    <VStack className="feed-page" data-component-page="feed" gap="6">
      ...
      <Scenario {...feedScenarios[1]}><VStack gap="4"><Cell label="two-part composition"><ActivityStream prefix="anatomy" /></Cell><RenderedOutput label="Rendered Feed HTML"><ActivityStream prefix="output" /></RenderedOutput></VStack></Scenario>
      ...
      <Scenario {...feedScenarios[8]}><EvidenceSurface as="article" className="feed-customization" data-testid="feed-customized" inset="lg"><VStack gap="2"><Badge size="sm" tone="accent">Customized</Badge><Text as="h3" variant="title-sm">Accent activity stream</Text><Text as="p" tone="secondary" variant="body-sm">Documented Feed variables recolor surfaces and separators while the rounded outline contains every article background.</Text><PlaygroundCodeBlock tabIndex={0}>{`--brick-feed-background: var(--brick-color-accent-soft);\n--brick-feed-border-color: var(--brick-color-accent-border);\n--brick-feed-divider-color: var(--brick-color-accent-border);\n--brick-feed-radius: 1rem;`}</PlaygroundCodeBlock></VStack><ActivityStream items={updates.slice(0, 2)} prefix="custom" style={customTokens} variant="outline" /></EvidenceSurface></Scenario>
