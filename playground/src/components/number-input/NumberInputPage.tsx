...
import { Button, Field, Form, Grid, NumberInput, Text, VStack, type NumberInputShape, type NumberInputSize, type NumberInputVariant } from "@flowstack-ui/brick";
...
const sizes: NumberInputSize[] = ["2xs", "xs", "sm", "md", "lg", "xl", "2xl"];
const shapes: NumberInputShape[] = ["sharp", "rounded", "pill"];
const definitions = [
  [1, "Overview", "Canonical 44px lg outline Number Input composed with Field."],
...
export function NumberInputPage() {
  ...
  return <VStack className="forms-page specialized-field-page" data-component-page="number-input">
    ...
    <Scenario {...numberInputScenarios[4]}><Grid.Root columns={{ initial: 1, md: 3 }} className="forms-grid" data-testid="number-input-stepping"><Cell label="decimal step with bounds"><Labeled id="number-step"><Control defaultValue={1.5} min={0} max={2} precision={1} step={0.5} /></Labeled></Cell><Cell label="hover steppers"><Labeled id="number-hover-step"><Control defaultValue={12} stepperVisibility="hover" /></Labeled></Cell><Cell label="square stepper"><Labeled id="number-square-step"><NumberInput.Root defaultValue={3} layout="stepper" min={1} max={9}><NumberInput.Decrement aria-label="Remove quantity"/><NumberInput.Input/><NumberInput.Increment aria-label="Add quantity"/></NumberInput.Root></Labeled></Cell></Grid.Root></Scenario>
    ...
    <Scenario {...numberInputScenarios[7]}><VStack className="forms-evidence-stack" data-testid="number-input-appearance"><EvidenceGroup title="Scoped appearances" description="Compact badges identify identical Number Input and Field defaults inside light and dark scopes."><Grid.Root columns={2} className="forms-scoped-grid"><EvidenceSurface data-brick-appearance="light"><SpecimenLabel>Light</SpecimenLabel><Labeled id="number-light"><Control defaultValue={3} /></Labeled></EvidenceSurface><EvidenceSurface data-brick-appearance="dark"><SpecimenLabel>Dark</SpecimenLabel><Labeled id="number-dark"><Control defaultValue={3} /></Labeled></EvidenceSurface></Grid.Root></EvidenceGroup><EvidenceGroup title="Consumer customization" description="The titled preview uses only the documented public wrapper variables shown beside it."><EvidenceSurface as="article" className="forms-customization" inset="lg"><div><SpecimenLabel>Customized</SpecimenLabel><Text as="h4" variant="title-sm">Inventory accent</Text><Text as="p" tone="secondary" variant="body-sm">The green surface, border, and radius are scoped to one Number Input.</Text><PlaygroundCodeBlock tabIndex={0}>{`--brick-number-input-background: #eefbf5;\n--brick-number-input-border: #18794e;\n--brick-number-input-radius: 0.75rem;`}</PlaygroundCodeBlock></div><EvidenceSurface className="forms-customization__preview"><Labeled id="number-custom"><Control defaultValue={3} style={customTokens} /></Labeled></EvidenceSurface></EvidenceSurface></EvidenceGroup></VStack></Scenario>
