...
function RtlProjectGrid() {
  const rtlRows = [{ id: "atom", project: "أتوم", status: "جاهز", checks: 492 }, { id: "brick", project: "بريك", status: "قيد المراجعة", checks: 238 }, { id: "consumer", project: "المستهلك", status: "جاهز", checks: 24 }];
  return <DataGrid.Root aria-label="نتائج التحقق من الحزم" columnCount={3} dir="rtl" rowCount={4} variant="outline"><DataGrid.Caption side="top">نتائج التحقق من الحزم</DataGrid.Caption><DataGrid.Header><DataGrid.Row rowIndex={1}><DataGrid.ColumnHeader columnIndex={1}>الحزمة</DataGrid.ColumnHeader><DataGrid.ColumnHeader columnIndex={2}>الحالة</DataGrid.ColumnHeader><DataGrid.ColumnHeader columnIndex={3} numeric>الفحوصات</DataGrid.ColumnHeader></DataGrid.Row></DataGrid.Header><DataGrid.Body>{rtlRows.map((row, index) => <DataGrid.Row key={row.id} rowIndex={index + 2} selectable value={row.id}><DataGrid.Cell columnIndex={1}>{row.project}</DataGrid.Cell><DataGrid.Cell columnIndex={2}><Badge size="sm" tone={row.status === "جاهز" ? "success" : "warning"}>{row.status}</Badge></DataGrid.Cell><DataGrid.Cell columnIndex={3} numeric>{row.checks}</DataGrid.Cell></DataGrid.Row>)}</DataGrid.Body></DataGrid.Root>;
}
...
export function DataGridPage() {
  ...
  return <VStack className="data-grid-page" data-component-page="data-grid" gap="6">
    ...
    <Scenario {...dataGridScenarios[7]}><VStack gap="5"><Grid.Root className="data-grid-specimens" columns={2} gap="4"><EvidenceSurface data-brick-appearance="light"><SpecimenLabel>light</SpecimenLabel><ProjectGrid variant="outline" /></EvidenceSurface><EvidenceSurface data-brick-appearance="dark"><SpecimenLabel>dark</SpecimenLabel><ProjectGrid variant="outline" /></EvidenceSurface></Grid.Root><EvidenceSurface className="playground-customization-evidence" inset="none"><Grid.Root className="data-grid-customization playground-customization-layout" columns={2} gap="0"><VStack gap="2"><SpecimenLabel>customized</SpecimenLabel><Text as="h3" variant="title-sm">Data Grid CSS properties</Text><Text tone="secondary" variant="body-sm">The preview uses the exact accent border, header surface, and rounded corners shown in code. Navigation and selection behavior remain unchanged.</Text><PlaygroundCodeBlock tabIndex={0}>{`--brick-data-grid-border-color: var(--brick-color-accent-border);\n--brick-data-grid-header-background: var(--brick-color-accent-subtle);\n--brick-data-grid-radius: 1rem;`}</PlaygroundCodeBlock></VStack><div className="playground-customization-preview"><ProjectGrid style={customStyle} variant="outline" /></div></Grid.Root></EvidenceSurface></VStack></Scenario>
    <Scenario {...dataGridScenarios[8]}><VStack gap="4"><Text tone="secondary" variant="body-sm">Data Grid owns cell navigation and selection. Table is static, Grid is layout, and Tree Grid adds hierarchy.</Text><Cell label="responsive overflow"><DataGrid.Container><ProjectGrid style={{ "--brick-data-grid-inline-size": "52rem", "--brick-data-grid-min-inline-size": "52rem" } as CSSProperties} /></DataGrid.Container></Cell><Cell label="RTL localized content"><div dir="rtl"><DataGrid.Container><RtlProjectGrid /></DataGrid.Container></div></Cell></VStack></Scenario>
  </VStack>;
}
