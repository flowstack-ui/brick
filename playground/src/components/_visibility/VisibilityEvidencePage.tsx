...
export function createVisibilityScenarios(id: "show" | "hide") {
  ...
  return [
    ...
    { id: `${id}.display`, number: 4, title: "Native display preservation", description: "Visible hosts retain authored display, while Show can explicitly remove a layout-interrupting wrapper box." },
...
export function VisibilityEvidencePage({ component: Component, id, scenarios }: { component: ComponentType<VisibilityProps & RefAttributes<HTMLElement>>; id: "show" | "hide"; scenarios: readonly ScenarioDefinition[] }) {
  ...
  return <VStack className="visibility-page" data-component-page={id} gap="6">
    ...
    <Scenario {...scenarios[3]}><Grid.Root data-testid={`${id}-display`} gap="3" minItemSize="sm"><Card label="block child"><Component from="md"><div style={{ display: "block" }}>Block</div></Component></Card><Card label="inline child"><Component from="md"><span style={{ display: "inline" }}>Inline</span></Component></Card><Card label="flex child"><Component from="md"><div style={{ display: "flex" }}>Flex</div></Component></Card><Card label="grid child"><Component from="md"><div style={{ display: "grid" }}>Grid</div></Component></Card></Grid.Root></Scenario>
