...
function Appearance({ appearance }: { appearance: "light" | "dark" }) {
  return <EvidenceSurface className="aspect-ratio-appearance" data-brick-appearance={appearance}><SpecimenLabel>{appearance === "light" ? "Light" : "Dark"}</SpecimenLabel><Frame radius="lg" variant="outline" /></EvidenceSurface>;
}
...
export function AspectRatioPage() {
  ...
  return <VStack className="aspect-ratio-page" data-component-page="aspect-ratio" gap="6">
    ...
    <Scenario {...aspectRatioScenarios[6]}><VStack gap="4"><RenderedOutput label="Aspect Ratio rendered output"><AspectRatio.Root aria-label="Native preview" className="aspect-ratio-rendered-article" data-evidence="native" ratio={1} ref={ref} render={(props) => <article {...props}><Text variant="body-sm">Rendered article</Text></article>} /></RenderedOutput><HStack gap="3"><Button onClick={() => setRefResult(ref.current?.tagName ?? "Missing")} size="sm" variant="outline">Inspect ref</Button><Text aria-live="polite" variant="body-sm">Ref host: {refResult}</Text></HStack><AspectRatio.Root asChild ratio={1}><section aria-label="Composed square" className="aspect-ratio-composed" /></AspectRatio.Root><AspectRatio.Root className="aspect-ratio-invalid" data-testid="aspect-ratio-invalid" ratio={0}><div className="aspect-ratio-content"><Text variant="body-sm">Invalid ratio fallback</Text></div></AspectRatio.Root></VStack></Scenario>
    <Scenario {...aspectRatioScenarios[7]}><VStack data-testid="aspect-ratio-appearance" gap="4"><Grid.Root className="aspect-ratio-grid" columns={2} gap="4"><Appearance appearance="light" /><Appearance appearance="dark" /></Grid.Root><EvidenceSurface className="playground-customization-evidence" inset="none"><Grid.Root className="playground-customization-layout" columns={2} gap="0"><VStack gap="2"><SpecimenLabel>Customized</SpecimenLabel><Text as="h3" variant="title-sm">Aspect Ratio CSS properties</Text><Text tone="secondary" variant="body-sm">Accent frame paint and a one-rem radius come only from public variables.</Text><PlaygroundCodeBlock aria-label="Aspect Ratio customization example">{`.preview {\n  --brick-aspect-ratio-background: var(--brick-color-accent-subtle);\n  --brick-aspect-ratio-border-color: var(--brick-color-accent-border);\n  --brick-aspect-ratio-radius: 1rem;\n}`}</PlaygroundCodeBlock></VStack><div className="playground-customization-preview"><Frame style={customStyle} variant="outline" /></div></Grid.Root></EvidenceSurface></VStack></Scenario>
