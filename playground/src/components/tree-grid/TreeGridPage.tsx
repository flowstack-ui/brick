...
function FileGrid({ rows = files, caption = "Release files", captionSide = "bottom", footer = true, ...props }: React.ComponentProps<typeof TreeGrid.Root> & { rows?: FileRow[]; caption?: string; captionSide?: "top" | "bottom"; footer?: boolean }) {
  return <TreeGrid.Root {...props} aria-label={caption} columnCount={3} rowCount={rows.length + 1 + (footer ? 1 : 0)} defaultExpandedValue={["src", "components"]}><TreeGrid.ColumnGroup><TreeGrid.Column htmlWidth="50%" /><TreeGrid.Column /><TreeGrid.Column htmlWidth="18%" /></TreeGrid.ColumnGroup>
...
export function TreeGridPage() {
  ...
  return <VStack className="tree-grid-page" data-component-page="tree-grid" gap="6">
    ...
    <Scenario {...treeGridScenarios[2]}><Grid.Root className="tree-grid-specimens" columns={3} gap="4">{variants.map(variant => <Cell key={variant} label={variant}><TreeGrid.Container><FileGrid variant={variant} /></TreeGrid.Container></Cell>)}<Cell label="base surface"><TreeGrid.Container><FileGrid surface="base" /></TreeGrid.Container></Cell><Cell label="striped"><TreeGrid.Container><FileGrid striped /></TreeGrid.Container></Cell><Cell label="subtle outline + columns"><TreeGrid.Container><FileGrid borderTone="subtle" showColumnBorder variant="outline" /></TreeGrid.Container></Cell><Cell label="strong sticky outline"><TreeGrid.Container><FileGrid borderTone="strong" stickyHeader variant="outline" /></TreeGrid.Container></Cell></Grid.Root></Scenario>
