...
2. Use arrows, `Home`, `End`, and typeahead; expect focus to move among enabled
   options without selecting Enterprise.
3. Press `Enter` or `Space`; expect one option to be selected and focus to
...
1. With macOS VoiceOver, read the Overview Field and trigger; expect the label,
   ...
   applicable.
2. Open and traverse options; expect group labels, selected state, and disabled
   Enterprise to be announced without decorative artwork.
...
1. With a mouse, open Overview, choose Starter, reopen, and click outside;
   expect selection and dismissal after the click.
2. On real touch hardware, tap outside an open Select; expect dismissal only
   after release.
3. Reopen, begin outside contact, then drag or scroll; expect Select to remain
...
1. In scenario 07, submit the default form; expect output `team`.
2. Choose Starter, submit, and expect `starter`; reset and expect the original
   value plus `Form reset` output.
...
4. Open the asChild example; expect the composed button to retain Select
...
3. Test light, dark, reduced motion, and forced colors; expect visible focus,
   selected, highlighted, disabled, read-only, and invalid states.
